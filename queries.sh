...
#echo "$($PSQL "select name from games as g left join teams as t on g.winner_id = t.team_id or g.opponent_id = t.team_id where (g.year=2014 and g.round='Eighth-Final') order by name")"
...
echo "$($PSQL "select distinct(name) from games as g left join teams as t on g.winner_id = t.team_id or g.opponent_id = t.team_id where name like 'Co%' order by name")"